...
export async function hasProfesorRole(user: any): Promise<boolean> {
  if (!user) return false;
  try {
    if (typeof user.role === 'string' && user.role.trim().toUpperCase() === 'PROFESOR') {
      return true;
    }
    ...
    if (Array.isArray(user.roles)) {
      return user.roles.some((r: any) => {
        const code = typeof r === 'string' ? r : (r && r.code ? r.code : '');
        return String(code).trim().toUpperCase() === 'PROFESOR';
      });
    }

    return false;
  } catch (err) {
    logger.error('Error checking profesor role in hasProfesorRole:', err);
    return false;
  }
}
...
export async function requireAdminOrProfessor(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    if ((await hasAdminRole(user)) || (await hasProfesorRole(user))) {
      return next();
    }
